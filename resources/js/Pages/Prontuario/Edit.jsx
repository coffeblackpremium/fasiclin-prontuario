...
export default function Edit({prontuario, cliente}) {
    return (
        <div className={"md:h-screen bg-green-600 dark:bg-gray-900"}>
            <Navbar />
            <div className="mx-auto pt-24">
                <h2 className="text-3xl text-center mb-4 font-bold dark:text-white">Prontuário do Paciente: {cliente.nome_cliente}</h2>
            </div>
                ...
                <form action="" className={"mt-5 md:mt-0"}>
                    <div className="md:grid md:grid-cols-2 md:gap-3 md:content-start flex flex-col justify-center">
                        <div className={"items-center max-w-xl"}>
                            <InputLabel for={"id_cliente"} value="ID Cliente"/>
                            <TextInput type="text" name="id_cliente" id={"id_cliente"} className={"block w-full mt-1 py-1"}
                                       value={prontuario.data[0].cpf_cliente} disabled>
                            </TextInput>
                            <InputError />
                        </div>
                        <div className={"max-w-xl items-center"}>
                            <InputLabel for="id_especialidade" value="ID Especialidade" />
                            <TextInput type="text" name="id_especiliadade" id="{id_especialidade}" className={"block w-full mt-1 py-1"}
                                       value={prontuario.data[0].id_especialidade} disabled/>
                            <InputError />
                        </div>
                    </div>
                    <div className={"md:grid md:grid-cols-3 md:gap-4 mt-3 flex flex-col justify-center"}>
                        <div className={"items-center max-w-xl"}>
                            ...
                            <TextInput type="text" name="{id_cliente}" className={"block w-full mt-1 py-1"}
                                       value={prontuario.data[0].id_profissional_de_saude} disabled/>
                        </div>
                        <div className={"items-center max-w-xs md:px-4 md:mx-4"}>
                            <InputLabel for="id_data" value="Data"/>
                            <TextInput type={"prontuario.data[0].data_hora" ?? "date"} name="id_data" className={"block w-full mt-1 py-1"}
                                       value={prontuario.data[0].data_abertura} disabled/>
                        </div>
